...
n_subjects_1 <- 30
n_subjects_2 <- 30
sample_data_1 <- rnorm(n = n_subjects_1, mean = 10.76, sd = 1.5)
sample_data_2 <- rnorm(n = n_subjects_2, mean = 10.5, sd = 1.5)

# the function cohen.d comes from the package effsize 
...
(p.calc.large <- pwr.t.test(power = 0.8, d=0.5, sig.level = 0.05, type = "paired")) 
...
# 1. Compute a bootstrap confidence interval for Cohen's d on the effect size between the Noise - White and Noise - Yellow condition from this data set for postAx and postRE.

dat <- read.csv('https://raw.githubusercontent.com/hashtagcpt/biostats2/master/Monitor_Birds.csv')

# 2. Compute a power plot for an ANOVA for two levels of power 80% and 50% for effect sizes of .1 through .5 -- plot both of them on the same graph. HINT: you'll need a categorical variable for color to distinguish the higher power (.8) from the lower power (.5)

#### Lecture 8 - Psychometric Function fitting ####
library(R.matlab)
library(tidyverse)
library(psyphy) # you'll probably need to install.packages() this... needed for the mAFC logit... more on that later. 
...
data <- data.frame(d_file$d)
# rename the columns to something more human-readable
colnames(data) <- (c('contrast','stim_int','resp_int','correct','blocks','age'))

# We need the unique contrast levels, to collect the psychometric function and by extension contrast threshold, we ran a number of trials at a given contrast level...
contrasts <- unique(data$contrast)

# Use tidyverse functions to tally up our data and summarize it
df_summary <- data %>% group_by(contrast,correct) %>% tally() %>%  ungroup() %>% complete(contrast, correct, fill = list(n = 1))

df_summary <- subset(df_summary, correct == 1)
df_summary$incorrect <- n_trials - df_summary$n
df_summary$correct <- df_summary$n

# to make our life easier let's turn negative contrast values positive
df_summary$contrast <- abs(df_summary$contrast)

# fitting a psychometric function is squarely in the realm of regression
psymet_fit <- glm(formula = cbind(correct, incorrect) ~ log(contrast), family = binomial(mafc.logit(m_choice)), data = df_summary)
...
psymet_plot <- ggplot(data = df_summary, aes(x = contrast, y = correct / n_trials)) + geom_point(size = 4) + geom_line(data = psymet_pred_df, aes(x = contrast, y = fit), size = 2) + ylab('proportion correct') + theme(text = element_text(size=16), axis.text.x = element_text(size = 16), axis.text.y = element_text(size=16))

psymet_plot

# The package modelfree gives us a nice easy threshold level from our glm prediction data
#install.packages('modelfree')
library(modelfree)
thresh_level = .75
threshold_slope(psymet_pred_df$fit, psymet_pred_df$contrast, thresh_level) 

# Exercise - Bootstrapping the Psychometric (Hint: this is really the first question on the last assignment)...

# With a standard psychometric function fit, we get one estimate of threshold. We can use and researchers often do, compute bootstrap confidence intervals on the psychometric function threshold. # This way we have an evaluation of the the variability of the observer's threshold. How do we do this, what do we resample?
#
# We need to resample our original datafile. You have two choices here, you can resample the data at each level of contrast (create a bootstrap resample that from the data at that point) or resample from the data across the whole data set. For the second option, you will now have an *unequal* number of trials at each contrast level. Today the goal is to do the first option, while the second option will be left for an assignment question. 
#
...
# Let's play around with this using the bird data.
dat <- read.csv('https://raw.githubusercontent.com/hashtagcpt/biostats2/master/Monitor_Birds.csv')
...
ttestBF(x = dat$postAx[dat$condition == 'Noise'], y = dat$postAx[dat$condition == 'Flicker'], nullInterval = c(0,1))

# We can also test color...
ttestBF(x = dat$postAx[dat$Light == 'Yellow'], y = dat$postAx[dat$Light == 'White'], nullInterval = c(0,1))

# Why make life difficult? Isn't this just the same as NHST? From this procedure we get the likelihood of our nullInterval our alt. interval generated our data. We're not a using conventional cut-off (i.e., alpha) but can quantify the possibilities. (NB ... but some folks want to categorize so Bayes factors as more interesting... life goes on... ) 
