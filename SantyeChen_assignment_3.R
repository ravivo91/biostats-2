...
Ex3_data <- read.csv('mini_data.csv')
boot.cor <- function(data, i) {
  d <- data[i,]
  summed <- sqrt(d[,2]^2 + d[,3]^2)
  cor_sum <- cor.test(x=d[,4], y=summed)
  return(cor_sum$estimate)
}
bootResults <- boot(statistic = boot.cor, data = Ex3_data, R = 1000)
boot.ci(bootResults, type = "perc", index =1) 
